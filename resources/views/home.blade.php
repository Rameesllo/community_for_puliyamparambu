            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                @forelse($teamMembers as $index => $member)
                    ...
                    <div class="card-hover bg-white rounded-2xl p-6 text-center shadow-sm border border-slate-100">
                        ...
                        @elseif($member->image)
                            <img src="{{ asset($member->image) }}" alt="{{ $member->name }}" class="w-20 h-20 rounded-full mx-auto mb-4 object-cover shadow-lg border-2 border-white" />
                        @else
                        ...
                        @if(($member->section ?? 'team') === 'founders')
                            <span class="inline-block px-2.5 py-0.5 mt-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-700">Founder</span>
                        @elseif(($member->section ?? 'team') === 'alumni')
                            <span class="inline-block px-2.5 py-0.5 mt-1 rounded-full text-xs font-semibold bg-amber-100 text-amber-700">Alumni</span>
                        @endif
